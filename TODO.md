...
- [ ] 🔴 清理与实际不符的目录结构与未使用文件 / 依赖（README 结构图同步更新）
- [ ] 🔴 完善 ESLint / Prettier 配置并全量通过
- [ ] 🟡 补关键模块单元测试（vitest）：录制、rrt 构建、平台提交、AI 上下文
- [ ] 🟡 复杂逻辑补 JSDoc / 注释，统一命名规范
- [ ] 🟡 建立 CI：GitHub Actions 跑 `lint + type-check + build + test`
- [ ] 🟡 提供 issue / PR 模板、CONTRIBUTING 说明
- [ ] 🟢 隐私与数据脱敏声明写入文档（录制内容可能含敏感信息）

## 5️⃣ 推广 & 涨粉（star / 公众号粉丝）

- [ ] 🔴 完善 README：产品截图、录制 / 回放 GIF 演示、功能亮点、徽章
- [ ] 🔴 录制宣传短视频（操作演示 + 价值点）投放：视频号、抖音、B站
- [ ] 🔴 掘金发布技术文章（rrweb 录制原理、网络 / 控制台采集、rrt 格式、MV3 扩展开发）
- [ ] 🔴 微信公众号发布文章 + 文末引导关注（每篇文章带项目链接）
- [ ] 🟡 建立落地页 / 官网（可复用 VitePress 首页）
- [ ] 🟡 提交到扩展商店并积极回复评论，引导到 GitHub
- [ ] 🟡 找早期用户内测，收集反馈（README 增加反馈入口）
