...
前往 **Releases** 页面下载最新的 `BugReplay.crx`，按下面步骤导入浏览器：

1. 打开扩展管理页：Chrome 访问 `chrome://extensions/`，Edge 访问 `edge://extensions/`
2. 开启右上角 **开发者模式**
3. 将 `.crx` 文件**拖拽**到该页面，按提示点击「添加扩展」
   - 若拖拽无反应或报错（如 `CRX_HEADER_INVALID`、`CRX_REQUIRED_PROOF_MISSING`），请改用下方「解压后加载」方式

> **导入注意事项**
>
> - **必须开启开发者模式**：Chrome 禁止直接安装非应用商店来源的 `.crx`，不开开发者模式无法导入
> - **推荐「解压后加载」**：新版 Chrome 对拖拽 `.crx` 支持不稳定，最稳做法是将 `.crx` 后缀改为 `.zip` 并解压到固定文件夹，再点击「加载已解压的扩展程序」选择该目录
> - **解压后的目录不要删除或移动**：浏览器运行时依赖该目录，移动会导致扩展失效
> - **`.crx` 可能被自动删除**：安全浏览或杀毒软件可能将 `.crx` 识别为风险文件并隔离，下载时留意浏览器提示
> - **安装后需刷新页面**：已打开的网页不会自动注入扩展脚本，需刷新（或重开）目标网站后页面内按钮才生效
> - **无痕 / 访客模式无法安装**扩展
> - 当前仅支持 **Chromium 内核**浏览器（Chrome / Edge / 360 / 搜狗等），且为 **Manifest V3**，Chrome 138+ 亦可正常加载
