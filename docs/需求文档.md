...
| 层 | 技术选型 | 说明 |
|----|---------|------|
| DOM 录制/回放引擎 | **rrweb v2** | 业界成熟的 DOM 快照与事件流录制方案 |
| 回放端 UI | **Vue 3 + Vant 4** | 组件化开发，暗色主题回放界面 |
...
| 标注/光标渲染 | **Fabric.js v6** | Canvas 2D 标注图层 + 回放自定义光标 |
| 网络拦截 | **注入式 <script>** | 注入页面主世界拦截 XHR/Fetch，突破隔离世界限制 |
| Cookie 采集 | **chrome.cookies API** | 获取完整 Cookie 属性（domain/path/expires/secure/httpOnly） |
| 状态管理 | **Pinia** | Vue 状态管理 |
| 样式 | **SCSS + UnoCSS** | 原子化 CSS + 嵌套样式 |
...
- **注入方式**：通过 `<script src>` 注入到页面主世界（非 Content Script 隔离世界），确保能拦截页面真实的 XHR/Fetch 请求
- **XHR 拦截**：代理 `XMLHttpRequest.prototype.open` / `send` / `setRequestHeader`，记录请求与响应头/体
- **Fetch 拦截**：代理 `window.fetch`，记录请求与响应
- **通信方式**：`window.postMessage` 将拦截数据从主世界传回 Content Script
- 采集字段：
...
- 存储状态：Cookies（含 domain/path/expires/secure/httpOnly/sameSite）、LocalStorage、SessionStorage
- 采集时机：录制开始时一次性快照（Cookie 通过 `chrome.cookies.getAll()` 获取完整属性）

#### 3.1.5 页面事件追踪 🆕

- **URL 变更**：追踪 `popstate`/`hashchange` 事件 + monkey-patch `history.pushState`/`replaceState`
- **Storage 变更**：monkey-patch `localStorage`/`sessionStorage` 的 `setItem`/`removeItem`/`clear`
- 所有变更记录时间戳，形成 `pageEvents[]` 时间轴
- 回放端在"时间轴"面板中可视化展示
...
- 录制控制：暂停/继续、停止、计时器显示
- **颜色选择器**：预设 6 种颜色（红/橙/黄/绿/蓝/紫），可实时切换 ✅
...
- 解析 .rrt 文件，提取各数据段
- 使用 rrweb Replayer 渲染 DOM 回放，Vue 3 + Vant 4 构建回放 UI
- 自定义进度条（拖拽跳转 + hover 预览 + 拖拽圆点），替代 Vant Slider
- 自定义鼠标光标（SVG 紫色箭头），基于 rrweb mousemove 事件坐标实时渲染
- 内容等比缩放，自适应窗口大小

#### 3.4.2 播放控制条

- 播放/暂停按钮（38px 圆形紫色按钮，hover 缩放反馈）
- 自定义进度条（点击/拖拽跳转，hover 显示预览位置）
- 速度控制：0.5x / 1x / 2x / 4x
- 时间显示：`00:23 / 01:45`
- 逐帧前进/后退（±1s）
- 功能按钮：打开文件、标注开关、重播、面板开关
- 快捷键：← → 逐帧、Space 播放/暂停（待实现）

#### 3.4.3 信息面板

- **右侧面板 — 录制信息**：
  - 录制元数据：标题、时长、版本、导出时间、事件统计
  - 环境信息：页面标题、URL、平台、语言、录制时刻、分辨率、视口/DPR
  - 存储查看：Cookies（点击弹出详情，含 domain/path/secure/httpOnly）、LocalStorage、SessionStorage，支持查看和复制

- **右侧面板 — 时间轴** 🆕：
  - 垂直时间轴展示录制过程中的 URL 变更、Storage 变更
  - 紫色圆点 = URL 变更，绿色圆点 = Storage 变更
  - 显示变更前后的值

- **右侧面板 — 关键帧**：
  - 标注列表，显示序号、类型图标、时间戳，可点击跳转

- **底部面板 — 网络请求**：
  - Chrome DevTools 风格表格：名称(Method+URL)、状态、类型、耗时
  - 点击选中 → 右侧详情面板（标头/请求/响应 三个 tab）
  - 响应体支持 JSON 格式化按钮 `{ }`
  - 搜索过滤
  - 根据播放进度渐进渲染（只显示当前时间之前的请求）

- **底部面板 — 控制台日志**：
  - 按时间排列，颜色区分级别（log/warn/error）
  - 点击展开查看完整参数
  - 搜索过滤
  - 根据播放进度渐进渲染
...
- 开关按钮控制标注图层显示/隐藏
- 基于 Fabric.js Canvas 渲染标注
- 标注根据 timestamp（归一化为相对时间）在对应时间点渐显
...
- Chrome >= 88（Manifest V3 最低版本，实际测试 Chrome 148）
- Edge >= 88
- Firefox >= 109（Manifest V3 支持版本，待测试）
...
```
src/
├── background/          # Service Worker（消息路由 + 存储 + 导出）
│   ├── service-worker.ts
│   ├── storage-manager.ts    # IndexedDB CRUD
│   └── rrt-builder.ts        # .rrt 打包/下载/剪贴板
├── content/             # Content Script（注入到目标页面）
│   ├── content-script.ts     # 主入口，composable 模式
│   ├── composables/
│   │   ├── useRecorder.ts    # 录制协调（rrweb + 控制台）
│   │   └── useAnnotator.ts   # 标注协调（工具栏 + Canvas）
│   ├── recorder/
│   │   ├── rrweb-recorder.ts
│   │   ├── page-interceptor.js   # 注入页面主世界：网络 + URL + Storage 拦截
│   │   ├── console-interceptor.ts
│   │   └── environment-snapshot.ts
│   └── annotator/
│       ├── toolbar.ts        # 底部悬浮工具栏（可拖拽）
│       ├── canvas-layer.ts   # Fabric.js 标注画布
│       └── tools/            # rect / arrow / text / freehand
├── popup/               # 扩展弹出窗口（Vue 3 + Vant 4）
│   ├── App.vue
│   ├── composables/
│   │   ├── useRecording.ts
│   │   └── useSessions.ts
│   └── components/          # 录制控制 + 会话列表 + 底部操作栏
├── replayer/            # 回放页面（Vue 3 + Vant 4）
│   ├── App.vue
│   ├── composables/
│   │   ├── useRePlayer.ts   # 回放核心逻辑
│   │   └── annotation-overlay.ts
│   └── components/          # 播放器控制条 + 面板
