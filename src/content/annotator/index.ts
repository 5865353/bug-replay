...
export class Annotator {
    ...
    constructor(options: AnnotatorOptions) {
        ...
        this.toolbar = new Toolbar({
            ...
            onColorChange: () => {
                if (this.activeTool) {
                    const t = this.getActiveToolType();
                    if (t) this.switchTool(t);
                }
            },
            onUndo: () => this.undo(),
    ...
    show(): void {
        ...
        this.isVisible = true;
        this.canvasLayer.show(this.options.sessionId);
        this.toolbar.show();
    }
    ...
    private switchTool(type: AnnotationToolType | null): void {
        ...
        switch (type) {
            case 'rect':
                this.activeTool = new RectTool(opts);
                break;
            case 'arrow':
                this.activeTool = new ArrowTool(opts);
                break;
            case 'text':
                this.activeTool = new TextTool(opts);
                break;
            case 'freehand':
                this.activeTool = new FreehandTool(opts);
                break;
        }
