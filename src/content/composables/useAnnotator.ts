...
export function useAnnotator(hooks: AnnotatorHooks) {
    ...
    function switchTool(type: AnnotationToolType | null): void {
        ...
        switch (type) {
            case 'rect': activeTool = new RectTool(opts); break
            case 'arrow': activeTool = new ArrowTool(opts); break
            case 'text': activeTool = new TextTool(opts); break
            case 'freehand': activeTool = new FreehandTool(opts); break
        }
        activeTool?.activate()
    }
    ...
    function setPaused(): void { toolbar.setPaused() }
    function setResumed(): void { toolbar.setResumed() }
    function undo(): void { canvasLayer.undoLast() }
    function clearAll(): void { canvasLayer.clearAll() }
    function getAnnotations(): Annotation[] { return canvasLayer.toAnnotations() }
