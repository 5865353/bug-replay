...
export class Sidebar {
    ...
    private renderConsole(): void {
        ...
        if (logs.length === 0) {
            const msg = this.consoleLogs.length === 0
                ? '<div class="empty-state">📋 暂无控制台日志<br><small>录制时未捕获到 console 输出</small></div>'
                : '<div class="empty-state">无匹配的控制台日志</div>';
            this.contentEl.innerHTML = msg;
            return;
        }
    ...
    private renderNetwork(): void {
        ...
        if (logs.length === 0) {
            const msg = this.networkLogs.length === 0
                ? '<div class="empty-state">🌐 暂无网络请求<br><small>录制时未捕获到 XHR/Fetch 请求</small></div>'
